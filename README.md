...
- Notes, Avatar, Memes, GIFs, Write, and Carousel each have a Name and Copy table for a new message template. Saved templates are chosen from Select below that table. A template saved in one studio stays in that studio. They are stored in Supabase for the signed-in operator.
